...
//  DZ14Strings
//
//  Created by Vasilii on 08.01.17.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSInteger midelPartOfText = [text length] / 2; //находим средину текста (индекс)
    
    NSString *firstPart = [text substringToIndex:midelPartOfText]; // определяем текст до середины
    NSLog(@"Fist part this text: \n%@", firstPart);
    
    NSString *otherPart = [text substringFromIndex:midelPartOfText]; // текст после середины
    NSLog(@"Other part this text: \n%@", otherPart);
    
    NSArray *arrayText = [text componentsSeparatedByString: @". "]; // массив в виде предложений, как находит точку создает элемент массива
    NSLog(@"%@",arrayText);
    
#pragma mark - Student
    
    NSMutableArray *arrayNSString = [NSMutableArray array]; //Массив где будем хранить предложения со словом NSString"
    
    NSInteger counter = 1; // счетчик предложений
    
    for (NSString *word in arrayText) {
        NSRange range = [word rangeOfString:@"NSString"]; //поиск слова в каждом предложении
        if (range.location != NSNotFound) { // если слово найдено
            [arrayNSString addObject:word]; // добавляем предложение в массив
            NSLog(@"(%d) %@", counter, word);
            counter++;
        }
    }
